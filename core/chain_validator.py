...
class ChainValidator:
    ...
    def validate_chain(
        self, chain: Dict[str, Any], context: Dict[str, Any]
    ) -> ValidationResult:
        """
        ...
        result = ValidationResult(
            chain_id=chain_id, status=ChainStatus.INVALID, confidence=0.0
        )
        ...
        if missing_prereqs:
            ...
            result.recommendations.append(
                self._get_prereq_recommendation(missing_prereqs)
            )
    ...
    def validate_chains(
        self, chains: List[Dict[str, Any]], context: Dict[str, Any]
        ...
        for chain in chains:
            try:
                ...
                logger.error(
                    f"[CHAIN_VAL] Error validating chain {chain.get('name', 'unknown')}: {e}"
                )
                results.append(
                    ValidationResult(
    ...
    def _check_prerequisites(
        self, prerequisites: List[str], context: Dict[str, Any]
    ) -> Dict[str, bool]:
        ...
        for prereq in prerequisites:
            ...
            # Check vulnerability prerequisites
                ...
                met = any(
                    v.get("type", "").lower() in ["sql_injection", "sqli"]
                    for v in vulns
                )
            ...
            # Check authentication prerequisites
                ...
                met = context.get("has_auth", False) or context.get(
                    "authenticated", False
                )
    ...
    def _validate_steps(
        ...
        # FIX 2: Handle None steps
        valid_steps = []
        for i, step in enumerate(steps):
            if step is None:
                issues.append(f"Step {i + 1}: None step detected")
                continue
        ...
        steps = valid_steps
        if not steps:
            issues.append("Chain has no valid steps after filtering None entries")
            return False, issues
        ...
        for i, step in enumerate(steps):
            ...
            if action in [
                ...
                if not step.get("payload"):
                    issues.append(
                        f"Step {step_num}: Missing payload for action '{action}'"
                    )
    ...
    def _validate_step_url(self, url: str, step_num: int) -> Optional[str]:
        ...
        if not parsed.scheme:
            return (
                f"Step {step_num}: URL missing scheme (http:// or https://): {url[:80]}"
            )
    ...
    def _validate_chain_logic(self, chain: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if len(step_names) != len(set(step_names)):
            issues.append(
                "Chain has duplicate step names (possible circular dependency)"
            )
        ...
        if similar_endpoint_count > 10:
            issues.append(
                f"Chain has {similar_endpoint_count} similar endpoints (max 10 allowed)"
            )
    ...
    def _estimate_success_rate(
        ...
        if prereq_results:
            prereq_rate = sum(1 for met in prereq_results.values() if met) / len(
                prereq_results
            )
        ...
        historical_rate = self.chain_templates.get(chain_type, {}).get(
            "success_rate", 0.5
        )
    ...
    def _get_prereq_recommendation(self, missing_prereqs: List[str]) -> str:
        ...
        return (
            ...
            if recommendations
            else "Address missing prerequisites before execution"
        )

    def _load_chain_templates(self) -> Dict[str, Dict[str, Any]]:
        """Load chain templates: hardcoded baselines + rules/chains.json."""
        templates = {
            "wordpress_exploit": {"success_rate": 0.7, "description": "WordPress exploitation chain"},
            "sql_injection":     {"success_rate": 0.6, "description": "SQL injection exploitation"},
            "xss_to_session_hijack": {"success_rate": 0.4, "description": "XSS to session hijacking"},
            "file_upload_rce":   {"success_rate": 0.5, "description": "File upload to RCE"},
            "lfi_to_rce":        {"success_rate": 0.4, "description": "LFI to RCE chain"},
            "auth_bypass":       {"success_rate": 0.3, "description": "Authentication bypass"},
            "ssrf_to_internal_scan": {"success_rate": 0.6, "description": "SSRF to internal scanning"},
        }
        # Enrich with rules/chains.json
        try:
            ...
            if os.path.exists(rules_path):
                with open(rules_path) as f:
                    rules = json.load(f)
                for rule in rules:
                    key = rule.get("id", rule.get("name", "")).lower().replace(" ", "_")
                    if key:
                        priority = rule.get("priority", 3)
                        success_rate = max(0.3, 0.8 - (priority - 1) * 0.15)
                        templates[key] = {
                            "success_rate": success_rate,
                            "description": rule.get("name", key),
                            "actions": rule.get("actions", []),
                        }
                logger.debug(f"[CHAIN_VAL] Loaded {len(rules)} rule templates from chains.json")
        except Exception as e:
            logger.warning(f"[CHAIN_VAL] Could not load rules/chains.json: {e}")
        return templates
    ...
    def _save_validations(self, results: List[ValidationResult]):
        ...
        try:
            ...
            logger.debug(
                f"[CHAIN_VAL] Saved {len(data)} validations to {self.validation_file}"
            )
        except Exception as e:
    ...
    def get_chain_statistics(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "low_probability": sum(
                1 for r in results if r.status == ChainStatus.LOW_PROBABILITY
            ),
            "avg_confidence": sum(r.confidence for r in results) / len(results)
            if results
            else 0,
            "avg_success_rate": sum(r.estimated_success_rate for r in results)
            / len(results)
            if results
            else 0,
        }
